...
Item {
  ...
  property string lastValidIdentity: ""
  property bool lastValidHasMedia: false
  ...
  Timer {
    ...
    repeat: false
    onTriggered: {
      if (!root.activePlayer || root.isYouTube(root.activePlayer)) {
        root.lastValidTitle = ""
        root.lastValidArtist = ""
        root.lastValidAlbum = ""
        root.lastValidArtUrl = ""
        root.lastValidIdentity = ""
        root.lastValidHasMedia = false
      }
    }
  }
  ...
  function updateTrackInfo() {
    ...
    if (p && !root.isYouTube(p) && (p.trackTitle || p.trackArtist)) {
      clearMediaTimer.stop()
      root.lastValidTitle = p.trackTitle || ""
      root.lastValidArtist = p.trackArtist || ""
      root.lastValidAlbum = p.trackAlbum || ""
      root.lastValidArtUrl = p.trackArtUrl || ""
      root.lastValidIdentity = p.identity || p.desktopEntry || ""
      root.lastValidHasMedia = true
    } else {
      if (!clearMediaTimer.running && root.lastValidHasMedia) {
        clearMediaTimer.restart()
      }
    }
  }

  onActivePlayerChanged: updateTrackInfo()

  readonly property bool isCurrentYouTube: activePlayer ? isYouTube(activePlayer) : false
  readonly property bool hasMedia: (activePlayer !== null && !isCurrentYouTube && (activePlayer.trackTitle || activePlayer.trackArtist)) || (clearMediaTimer.running && lastValidHasMedia)
  readonly property string title: (activePlayer && !isCurrentYouTube && activePlayer.trackTitle) ? activePlayer.trackTitle : (clearMediaTimer.running ? lastValidTitle : "")
  readonly property string artist: (activePlayer && !isCurrentYouTube && activePlayer.trackArtist) ? activePlayer.trackArtist : (clearMediaTimer.running ? lastValidArtist : "")
  readonly property string album: (activePlayer && !isCurrentYouTube && activePlayer.trackAlbum) ? activePlayer.trackAlbum : (clearMediaTimer.running ? lastValidAlbum : "")
  readonly property string artUrl: (activePlayer && !isCurrentYouTube && activePlayer.trackArtUrl) ? activePlayer.trackArtUrl : (clearMediaTimer.running ? lastValidArtUrl : "")
  readonly property string identity: (activePlayer && !isCurrentYouTube) ? (activePlayer.identity || activePlayer.desktopEntry || "") : (clearMediaTimer.running ? lastValidIdentity : "")
  ...
  function isYouTube(player) {
    return MediaModel.isYouTube(player, root.toplevels)
  }

  function hasMetadata(player) {
    return MediaModel.hasMetadata(player, root.toplevels)
  }

  function hasTrackMetadata(player) {
    return MediaModel.hasTrackMetadata(player, root.toplevels)
  }

  function playerCanControl(player) {
    return MediaModel.playerCanControl(player, root.toplevels)
  }

  function canHandleAction(player, action) {
    return MediaModel.canHandleAction(player, action, root.toplevels)
  }

  function canCycleSource(player) {
    return MediaModel.canCycleSource(player, root.toplevels)
  }
