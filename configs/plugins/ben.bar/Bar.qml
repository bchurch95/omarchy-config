...
Item {
  ...
  function moduleClickTargetAt(slot, localX, localY) {
    ...
    for (var i = clickTargets.length - 1; i >= 0; i--) {
      ...
      var belongsToSlot = isDescendantOf(target, slot)
      if (belongsToSlot) slotTargets.push(target)

      var tolerance = belongsToSlot ? 8 : 1
      if (targetPoint.x >= -tolerance && targetPoint.x <= target.width + tolerance &&
          targetPoint.y >= -tolerance && targetPoint.y <= target.height + tolerance) {
        return target
      }
    }

    if (slotTargets.length > 0) return slotTargets[0]
